...
### Step 3
Once you have confirmed that the workflow run is complete...
run the command 
```bash 
make upload
``` 
...
You can now process another file with new data generated by running `make data`, then uploading the new file using `make upload` then processing the new file using `make invoke`

Again all the files will be deleted from the input and invoke buckets, now there will be 2 processed files in the processed bucket.
...
Both the `input` and `invocation` buckets must be empty before uploading a .csv file to the `input` bucket and a .json file to the `invocation` bucket.

## Viewing results

   -  Go to AWS website

there will now be a file (or files) in the `processed` bucket on the AWS website with the same time/date stamped filename as the original file uploaded to the `input` bucket with the selected PII fields data replaced with `***`.
...
# Future enhancements
   ...
   -  A way to remove case sensitivity from PII fields
